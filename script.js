(() => {
  ...
  // ---------- Canvas sizing ----------
  // The canvas now renders at the ACTUAL displayed pixel size (instead of a
  // fixed 800x450 that CSS then crops/scales). This keeps the camera math
  // correct on every screen size/aspect ratio, so the player is always
  // exactly where the camera thinks it is.
  const rows = 14;
  const WORLD_H = rows * TILE; // 448 - the authored level's own height
  let camY = 0; // vertical offset so the ground stays anchored to screen bottom

  function resizeCanvas(){
    const dw = Math.floor(canvas.clientWidth);
    const dh = Math.floor(canvas.clientHeight);
    if(dw > 0 && dh > 0 && (canvas.width !== dw || canvas.height !== dh)){
      canvas.width = dw;
      canvas.height = dh;
      W = dw; H = dh;
      camY = Math.max(0, H - WORLD_H);
    }
  }
...
"GGGGGGGGGGGGGGGGGG....GGGGGGGGGGGGGG....GGGGGGGGGG....GGGGGGGGGG....GGGGGGGGGG....GGGGGGGG",
  ...
  function updateEnemies(){
    ...
    enemies = enemies.filter(e => e.y < WORLD_H+200);
  }
  ...
  function updateCamera(){
    ...
    camX = Math.max(0, Math.min(Math.max(0, levelWidth - W), target));
  }
